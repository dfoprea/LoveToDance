...
import { Heart, Share2, Facebook, MessageSquare, Link as LinkIcon, MessageCircle, X, Send, Instagram, Music2, EyeOff } from 'lucide-react';
...
function Combinations() {
  ...
  const filteredCombos = useMemo(() => {
    return autoComboData
      .filter(combo => {
        const itemSocial = socialData[combo.id] || { tags: [] };
        if (!isContentVisible(combo.id, itemSocial.tags)) return false;
  ...
  }, [filter, selectedYear, sortOrder, socialData, isContentVisible]);
  ...
  const handleAddComment = (e: React.FormEvent, comboId: string) => {
    ...
    if (user && bannedUsers.includes(user.name)) {
      addToast('Contul tău este suspendat.', 'error');
      return;
    }
    const itemData = socialData[comboId] || { likes: 0, likedBy: [], comments: [] };
    const comment = {
  ...
  return (
    <motion.div className="page-container" style={{ padding: '2rem 2rem' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <motion.div style={{ textAlign: 'center', marginBottom: '2rem' }} variants={fadeInUp} initial="hidden" animate="visible">
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem', maxWidth: '700px', margin: '0 auto' }}>Explorează ultimele mișcări predate la curs.</p>
      </motion.div>

      {/* 1. FEATURED SECTION */}
      {latestCombo && filter === 'All' && selectedYear === 'All' && (
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: '4rem', maxWidth: '1000px', margin: '0 auto 4rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
            <span className="badge" style={{ background: 'var(--primary)', padding: '0.5rem 1.2rem', fontSize: '0.9rem' }}>NOU la Curs</span>
            {(socialData[latestCombo.id]?.tags || []).includes('hidden') && <span className="badge" style={{ background: '#ff4444' }}>HIDDEN</span>}
          ...
          <div className="course-card-pro" style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '0', padding: 0, borderRadius: '32px', overflow: 'hidden', minHeight: '550px' }}>
            <div style={{ background: '#000', position: 'relative', minHeight: '500px' }}>
              <div style={{ position: 'absolute', inset: 0, zIndex: 1 }}>
                <video src={latestCombo.url} controls style={{ width: '100%', height: '100%', objectFit: 'cover' }} onDoubleClick={() => handleLike(latestCombo.id)} onClick={() => { if (activeCommentsId === latestCombo.id) setActiveCommentsId(null); }} />
              </div>
              <div className="media-interaction-bar" style={{ right: '1rem', bottom: '2rem', transform: 'none', gap: '1.2rem', zIndex: 100, opacity: activeCommentsId === latestCombo.id ? 0 : 1, pointerEvents: activeCommentsId === latestCombo.id ? 'none' : 'auto' }}>
            ...
            <div style={{ padding: '3rem', display: 'flex', flexDirection: 'column', justifyContent: 'center', background: 'var(--bg-card)' }}>
              <div style={{ color: 'var(--primary)', fontWeight: 800, textTransform: 'uppercase', fontSize: '0.8rem', letterSpacing: '2px', marginBottom: '1rem' }}>Actualizat: {latestCombo.date}</div>
              ...
              <p style={{ fontSize: '1.1rem', color: 'var(--text-muted)', lineHeight: '1.6', marginBottom: '2.5rem' }}>Aceasta este cea mai nouă coregrafie de {latestCombo.category}.</p>
              <button className="btn btn-primary btn-glow" style={{ alignSelf: 'flex-start', padding: '1rem 2rem' }} onClick={() => navigate('/contact')}>Vreau să învăț și eu</button>
            </div>
          </div>
        </motion.section>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '3rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
          <div className="filter-group-pro">
            ...
            {[...new Set(autoComboData.map(c => c.category))].map(cat => (<button key={cat} className={filter === cat ? 'active' : ''} onClick={() => setFilter(cat)}>{cat}</button>))}
          </div>
          <div className="filter-group-pro">
            {availableYears.map(year => (<button key={year} className={selectedYear === year ? 'active' : ''} onClick={() => setSelectedYear(year)} style={{ fontSize: '0.8rem', padding: '0.4rem 1rem' }}>{year === 'All' ? 'Toți anii' : year}</button>))}
          </div>
          <div className="filter-group-pro">
      ...
      <AnimatePresence mode="wait">
        <motion.div key={filter + selectedYear + sortOrder} className="responsive-grid" variants={staggerContainer} initial="hidden" animate="visible">
          {archiveCombos.map(combo => {
            ...
            const isHidden = (itemSocial.tags || []).includes('hidden');
            ...
            return (
              <motion.div key={combo.id} className="course-card-pro" variants={scaleUp} style={{ padding: '1rem', borderRadius: '24px', position: 'relative' }}>
                <div style={{ width: '100%', aspectRatio: '9 / 16', background: '#000', borderRadius: '16px', overflow: 'hidden', position: 'relative' }}>
                  <div style={{ position: 'absolute', inset: 0, zIndex: 1 }}>
                    <video src={combo.url} controls style={{ width: '100%', height: '100%', objectFit: 'cover' }} onDoubleClick={() => handleLike(combo.id)} onClick={() => { if (activeCommentsId === combo.id) setActiveCommentsId(null); }} />
                  </div>
                  <div className="media-interaction-bar" style={{ right: '0.8rem', bottom: '2rem', transform: 'none', gap: '1rem', zIndex: 100, opacity: activeCommentsId === combo.id ? 0 : 1, pointerEvents: activeCommentsId === combo.id ? 'none' : 'auto', transition: 'opacity 0.2s ease' }}>
                    <div className={`interaction-item ${isLikedByMe ? 'liked' : ''}`} onClick={(e) => { e.stopPropagation(); handleLike(combo.id); }}><div className="interaction-btn" style={{ width: '36px', height: '36px' }}><Heart size={18} fill={isLikedByMe ? "currentColor" : "none"} /></div><span className="interaction-count">{likes}</span></div>
                    <div className="interaction-item" onClick={(e) => { e.stopPropagation(); setActiveCommentsId(combo.id); }}><div className="interaction-btn" style={{ width: '36px', height: '36px' }}><MessageCircle size={18} /></div><span className="interaction-count">{comments.length}</span></div>
                    <div className="interaction-item" style={{ position: 'relative' }}>
                      ...
                      <AnimatePresence>
                        {activeShareId === combo.id && (
                          <motion.div className="share-menu-floating" style={{ bottom: '0', right: '45px' }} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} onClick={(e) => e.stopPropagation()}>
                            <button className="share-option" onClick={() => handleShare(combo.url, 'whatsapp')} title="WhatsApp"><MessageSquare size={14} /></button>
                            <button className="share-option" onClick={() => handleShare(combo.url, 'facebook')} title="Facebook"><Facebook size={14} /></button>
                            <button className="share-option" onClick={() => handleShare(combo.url, 'instagram')} title="Instagram"><Instagram size={14} /></button>
                            <button className="share-option" onClick={() => handleShare(combo.url, 'tiktok')} title="TikTok"><Music2 size={14} /></button>
                            <button className="share-option" onClick={() => handleShare(combo.url, 'copy')} title="Copy Link"><LinkIcon size={14} /></button>
                          </motion.div>
                        )}
                      </AnimatePresence>
                    </div>
                  </div>
                  {isHidden && <div style={{ position: 'absolute', inset: 0, background: 'rgba(255,68,68,0.2)', zIndex: 5, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}><EyeOff size={30} color="#fff" style={{ opacity: 0.5 }} /></div>}
                  <AnimatePresence>
                        ...
                        <div className="drawer-comments-list" ref={commentsListRef} style={{ paddingBottom: '0.5rem', textAlign: 'left' }}>
                          {comments.map((c: any, i: number) => (
                            <div key={i} className="comment-bubble" style={{ fontSize: '0.75rem', padding: '0.5rem 0.7rem', borderRadius: '12px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.05)', marginBottom: '0.4rem', textAlign: 'left' }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.2rem', alignItems: 'center' }}><strong style={{ color: 'var(--primary)', fontWeight: 800 }}>{c.user}</strong><span style={{ opacity: 0.5, fontSize: '0.55rem', color: '#fff' }}>{c.date}</span></div>
                ...
                <div style={{ padding: '1.5rem 0.5rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.7rem', color: isHidden ? '#ff4444' : 'var(--primary)', fontWeight: 800, marginBottom: '0.5rem' }}>
                    <span>{combo.category} {isHidden && '[HIDDEN]'}</span><span>{combo.date}</span>
                  </div>
                  <h3 style={{ fontSize: '1.2rem', fontWeight: 800, marginBottom: '1rem' }}>{combo.title}</h3>
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>{combo.tags.map(t => <span key={t} style={{ fontSize: '0.65rem', padding: '0.2rem 0.6rem', background: 'rgba(255,255,255,0.05)', borderRadius: '4px', color: 'var(--text-muted)' }}>#{t}</span>)}</div>
                </div>
              </motion.div>
            );
          })}
