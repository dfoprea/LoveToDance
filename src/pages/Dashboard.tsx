...
function Dashboard() {
  ...
  const analytics = useMemo(() => {
    ...
    dataArray.forEach(([id, data]: [string, any]) => {
      ...
      (data.tags || []).forEach((tag: string) => { if (tag !== 'hidden') tagStats[tag] = (tagStats[tag] || 0) + 1; });
    });
  ...
  const getCustomChartData = (xAxis: string, yAxis: string, legendAxis: string) => {
    ...
    Object.entries(socialData).forEach(([id, data]: [string, any]) => {
      ...
      if (commentsArr.length > 0) {
         commentsArr.forEach((c: any) => {
            ...
            if (parts.length >= 3) timePoints.push(`${parts[1].slice(0,3)} ${parts[2].slice(-2)}`);
         });
      } else { timePoints.push('Trecut'); }
      ...
      let weight = (xAxis === 'date' || legendAxis === 'date') ? (timePoints.length || 1) : 1;
    ...
    if (xAxis === 'date') {
       const monthsOrder: any = { 'Ian':1, 'Feb':2, 'Mar':3, 'Apr':4, 'Mai':5, 'Iun':6, 'Iul':7, 'Aug':8, 'Sep':9, 'Oct':10, 'Noi':11, 'Dec':12, 'Jan':1, 'May':5, 'Jun':6, 'Jul':7 };
       result.sort((a, b) => {
          const partsA = a.name.split(' ');
          const partsB = b.name.split(' ');
          if (partsA.length < 2 || partsB.length < 2) return 0;
          if (partsA[1] !== partsB[1]) return Number(partsA[1]) - Number(partsB[1]);
          return (monthsOrder[partsA[0]] || 0) - (monthsOrder[partsB[0]] || 0);
       });
    } else {
       result.sort((a, b) => {
  ...
  const resetTheme = () => {
    ...
    const defaultConfig = { primary: '#9b1c1c', secondary: '#e6e6e6', bgDark: '#0f0f11', bgCard: '#1a1a1e', textMain: '#f8fafc', textMuted: '#94a3b8', btnRadius: 10, cardRadius: 16 };
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'flex-end' }}>
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        {['salsa', 'bachata', 'kizomba'].map(cat => {
                          ...
                          return <button key={cat} onClick={() => toggleCategory(cat)} style={{ padding: '0.4rem 0.8rem', fontSize: '0.75rem', fontWeight: 700, borderRadius: '20px', border: isSelected ? '1px solid var(--primary)' : '1px solid rgba(255,255,255,0.1)', background: isSelected ? 'var(--primary)' : 'var(--bg-card)', color: '#fff', display: 'flex', alignItems: 'center', gap: '0.4rem', textTransform: 'capitalize' }}><div style={{ width: '14px', height: '14px', borderRadius: '3px', border: '1px solid rgba(255,255,255,0.5)', background: isSelected ? '#fff' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{isSelected && <CheckCircle size={10} color="var(--primary)" strokeWidth={3} />}</div>{cat}</button>;
                        })}
                      </div>
                    </div>
                  </div>
                      ...
                      {analytics.topMedia.map((m, i) => (
                        <div key={m.id} style={{ display: 'flex', gap: '1.5rem', alignItems: 'center', padding: '1rem', borderBottom: '1px solid var(--border)' }}>
                          ...
                          <div style={{ width: '60px', height: '80px', borderRadius: '8px', overflow: 'hidden', background: '#000', flexShrink: 0 }}>
                            {m.url ? (m.url.endsWith('.mp4') ? <video src={m.url + '#t=0.5'} style={{ width: '100%', height: '100%', objectFit: 'cover' }} muted /> : <img src={m.url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />) : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.2 }}><ImageIcon size={20}/></div>}
                          </div>
                          <div style={{ flex: 1 }}><div style={{ fontWeight: 800, fontSize: '0.85rem', marginBottom: '0.3rem' }}>{m.id}</div><div style={{ fontSize: '0.75rem', opacity: 0.5 }}>{m.likes} ❤️ | {m.comments} 💬</div></div>
                          <div style={{ textAlign: 'right' }}><div style={{ fontSize: '1.2rem', fontWeight: 900, color: 'var(--primary)' }}>{m.score}</div></div>
                        </div>
                      ))}
                    </div>
                  </div>
...
function ColorRow({ label, value, onChange }: any) {
  return <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontWeight: 700 }}>{label}</span><input type="color" value={value} onChange={e => onChange(e.target.value)} style={{ width: '30px', height: '30px', border: 'none', borderRadius: '50%', cursor: 'pointer' }} /></div>;
}
